...
impl HttpTransportLayerCreator {
    pub fn create(&self, pool_key: &PoolKey, parent_span: Span) -> Result<HttpTransportLayer> {
...
pub type HttpTransportLayerStream = Box<dyn CommonStreamTrait>;

impl<Req> tower::Service<Req> for HttpTransportLayer {
    type Response = TokioIo<HttpTransportLayerStream>;
    ...
    fn call(&mut self, _: Req) -> Self::Future {
        ...
        let fut = async move {
            ...
            Ok(TokioIo::new(Box::new(stream) as _))
        }
