                ...
                &self.direct_forward_traffic_detector
            {
                ...
                if direct_forward_traffic_detector.should_forward_directly(&request_info) {
                    ...
                    match self.typ {
                        ...
                        TransportLayerType::Tcp => DecodeResult::ContinueAsTngTraffic(
                            TransportLayerStream::Tcp(unmodified_stream),
                        ),
                        // If the transport layer is configured to http1 or http2, we need to check the http request.
                        TransportLayerType::Http => {
                            let websocket =
                                async_tungstenite::accept_async(unmodified_stream.compat())
                                    .await
                                    .context("Failed to accept websocket connection")?;

                            DecodeResult::ContinueAsTngTraffic(TransportLayerStream::Http(
                                Box::new(ws_stream_tungstenite::WsStream::new(websocket).compat()),
                            ))
                        }
                    }
                }
                ...
                match self.typ {
                    TransportLayerType::Tcp => {
                        DecodeResult::ContinueAsTngTraffic(TransportLayerStream::Tcp(in_stream))
                    }
                    TransportLayerType::Http => {
                        ...
                        DecodeResult::ContinueAsTngTraffic(TransportLayerStream::Http(Box::new(
