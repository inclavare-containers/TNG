...
| >= 2.5.0 | Starting from this version, remote attestation uses a pluggable provider layer. Wire formats for messages carrying evidence and attestation tokens are extended in a backward-compatible way. JSON configuration relies on defaults where possible to preserve compatibility; the main migration for existing configs is to use `as_type: "grpc"` instead of the former `as_is_grpc` boolean flag when talking to a gRPC Attestation Service. |
