...
| > 2.9.2 | **Breaking change**: The `tng tools ohttp dump --attest-request` flag is removed. It accepted a raw background-check challenge token (`backgroundcheck:<token>`) that was impractical to obtain via the tool. Use `tng tools ohttp dump --verify '<VerifyArgs json>'` instead, which builds the AS converter, mints the challenge token via `converter.get_nonce()`, fetches the key config with `attestation_info`, and verifies it inline. |
