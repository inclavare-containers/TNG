...
- **`verify_signer_transparency`** (boolean, 可选，默认为`false`)：设置为`true`时，COCO验证器将验证Trustee AS签发的JWT token中嵌入的`signer_transparency`声明。该机制将签名证书绑定到TEE证据和Rekor v2透明度日志条目，防止证书伪造。验证内容包括：证书DER SHA-256匹配、载荷SHA-256匹配以及本地Rekor v2检查点签名验证。启用后若声明缺失或无效，验证将直接失败。
