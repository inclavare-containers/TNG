...
- **`aa_type`** (string，可选，默认为"uds"): Attestation Agent类型。可选值："uds", "builtin", "restful"
- **`aa_addr`** (string，"uds"和"restful"类型必填): Attestation Agent的地址。"uds"类型填写unix socket路径（例如："unix:///run/confidential-containers/attestation-agent/attestation-agent.sock"）；"restful"类型填写 [api-server-rest](https://github.com/inclavare-containers/guest-components/tree/main/api-server-rest) 的 HTTP 基础 URL（例如："http://localhost:8006"）。
- **`tee`** (string，"restful"类型必填): TEE类型（例如："tdx"、"sgx"、"sample"）。当 `aa_type` 为"restful"时必须指定，因为 REST API 未提供 TEE 类型查询接口。
