...
- [远程证明（公共配置）](#远程证明公共配置)
  ...
  - [角色组合示例](#角色组合示例)
  - [Builtin AS PCCS 配置](#builtin-as-pccs-配置)
- [OHTTP 协议](#ohttp-协议)
...
使用 **SGX 或 TDX** 内置 AS 模式时，本地 Quote 验证过程需要从 **PCCS**（Provisioning Certificate Caching Service）获取并验证供应证书。PCCS URL 配置在 `/etc/sgx_default_qcnl.conf` 文件中。

如果 PCCS URL 未正确设置为你的云服务商地址，Quote 验证将失败并报错：

```
tee_verify_quote failed: 0xe019
```

#### 使用 `setup-vendor-config` 工具

TNG 提供了 `setup-vendor-config` 工具，可自动为支持的云服务商配置 `/etc/sgx_default_qcnl.conf`：

```sh
# 配置阿里云（默认区域：cn-hangzhou）
...
在启动 TNG 内置 AS 模式**之前**运行此工具。在 Docker 中，可以在容器内先运行该工具，再执行 `tng launch`：
...
> [!TIP]
> 阿里云的 `--region` 参数是可选的 —— `cn-hangzhou` 可服务于所有 TDX 实例。仅在你需要不同端点时才使用 `--region`。

#### 手动配置

如果你的服务商尚未被工具支持，可手动编辑 `/etc/sgx_default_qcnl.conf`：
...
将 `<region>` 和 `<provider-domain>` 替换为你的云服务商的 PCCS 端点。请参考云服务商的机密计算文档获取正确的 URL。
