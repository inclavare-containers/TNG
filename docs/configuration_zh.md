...
- **`verify_signer_transparency`** (boolean, 可选，默认为`false`)：设置为`true`时，COCO验证器将验证Trustee AS签发的JWT token中嵌入的`signer_transparency`声明。
  ...
  > [!NOTE]
  > **背景**：当Trustee AS运行在TEE内部时，其生成的JWT签名证书缺乏内生的可信机制——验证方无法确认该证书的真实性及其与TEE的绑定关系。`signer_transparency`功能通过在Trustee AS中发布透明度声明来解决此问题，该声明将签名证书与TEE证据（如TDX Quote）绑定，并记录到Rekor v2透明度日志中。当验证方启用此选项时，可确认证书确实由TEE保护的AS生成且已公开记录，从而防止证书伪造或中间人攻击。
  >
