...
- **`rats_tls`** (RatsTlsArgs, 可选)：rats-TLS传输配置。当既未指定`ohttp`也未指定`rats_tls`时，默认使用rats-TLS模式。
    - **`multiplex`** (boolean, 可选，默认为`false`)：当为`true`时，使用HTTP/2 CONNECT隧道在单个rats-TLS连接上复用多个TCP流，减少握手开销——适合大量短连接、每连接数据传输量小的场景。当为`false`（默认）时，每个下游连接创建独立的TLS会话，不使用HTTP/2 CONNECT或连接池，可获得更高的单流吞吐量——推荐用于高带宽场景。注意：开启`multiplex`时，所有流共享同一条TLS连接，其带宽受限于单核CPU的TLS加密能力。
...
### netfilter：端口劫持方式
在该场景中，用户的server程序已监听在本机某一端口，且因业务原因不便变更端口号或为tng新增开放端口。tng 通过内核 netfilter 拦截**来自其他节点发往该端口**的入站流量，将其重定向到 tng 监听的 `listen_port` 上。tng 解密流量后，将明文 TCP 流量发回到本机的原始 server 端口。

此模式用于捕获来自其他节点发往本节点端口的流量。

#### 字段说明
- **`capture_dst`** (array, 可选，默认为空数组)：指定需要被tng捕获的流量的目标地址和端口。如果未指定该字段或者指定为空数组，则所有TCP流量都将被捕获。

  该字段以数组形式包含多个对象，每个对象表示一条匹配规则，用于匹配TCP请求的目标地址和端口信息。具体规则字段如下：
...
> [!NOTE]
> **当前实现说明**：  
> 目前 TNG 支持通过 [Attestation Agent](https://github.com/confidential-containers/guest-components/tree/main/attestation-agent) (AA) 直接获取 Evidence（`"coco"` / `"ita"` Provider），或通过 [API Server Rest](https://github.com/confidential-containers/guest-components/tree/main/api-server-rest) (ASR) HTTP 代理间接获取（`"coco_asr"` / `"ita_asr"` Provider）。  
> ASR Provider 适用于 TNG 运行在无法直接访问 AA Unix socket 的容器中。
> **CoCo** 和 **ITA** Provider 都使用相同的 Attestation Agent 进行证据收集，但在运行时数据的处理和注入方式上有所不同（根据各自 attestation service 的要求）。
...
> **当前实现说明**：  
> 对于 **CoCo** Provider，Verifier 依赖于 [CoCo Attestation Service](https://github.com/confidential-containers/trustee/tree/main/attestation-service) 来验证接收到的 Evidence。它提供了统一的接口用于解析和验证不同 TEE 类型的证明数据，并支持与 Trustee Server 集成以实现集中式策略管理与信任根分发。  
> 对于 **ITA** Provider，Verifier 依赖于 Intel Trust Authority attestation service (https://www.intel.com/content/www/us/en/security/trust-authority.html) 来验证接收到的 Evidence。它可以解析和验证 TDX Confidential VM 和 Nvidia GPU 的证明数据，并支持策略管理和执行。
...
在Background Check模式下，[Attest](#attest)配置需要包含以下字段。以下字段适用于默认 CoCo Provider（`aa_provider` = `"coco"` 或省略）：

- **`model`** (string，可选): 设置为"background_check"以启用Background Check模式
- **`aa_type`** (string，可选，默认为"uds"): Attestation Agent类型。可选值："uds", "builtin"
- **`aa_addr`** (string，"uds"类型必填): Attestation Agent的unix socket地址（例如："unix:///run/confidential-containers/attestation-agent/attestation-agent.sock"）
...
或者，要通过 CoCo [API Server Rest](https://github.com/confidential-containers/guest-components/tree/main/api-server-rest) (ASR) HTTP 代理收集证据而不是直接连接到 AA（适用于 TNG 运行在容器中），将 `aa_provider` 设置为 `"coco_asr"` 并提供 **`asr_addr`**（ASR HTTP 地址，例如 `"http://127.0.0.1:8006"`）代替 `aa_addr`。
...
当 `aa_provider` 设置为 `"ita"` 时，Attest 配置使用以下字段：

- **`aa_provider`** (string)：设置为 `"ita"`
- **`aa_addr`** (string，必填)：Attestation Agent Unix socket 地址（例如：`"unix:///run/confidential-containers/attestation-agent/attestation-agent.sock"`）
- **`refresh_interval`** (int，可选，默认值为600)：与上述 CoCo Provider 的行为相同。

或者，将 `aa_provider` 设置为 `"ita_asr"` 并提供 **`asr_addr`** 代替 `aa_addr`，以通过 ASR HTTP 代理收集 ITA 证据。
...
在Background Check模式下，[Verify](#verify)配置需要包含以下字段。以下字段适用于默认 CoCo Provider（`as_provider` = `"coco"` 或省略）：

- **`model`** (string，可选): 设置为"background_check"以启用Background Check模式
- **`as_type`** (string，可选，默认为"restful"): Attestation Service类型。可选值："restful", "grpc", "builtin"
- **`as_addr`** (string，"restful"和"grpc"类型必填): Attestation Service的地址
...
- **`policy`** (object, "builtin"类型可选): 内置AS的OPA策略配置。详见[Builtin AS配置](#builtin-as-configuration)。
- **`reference_values`** (array, "builtin"类型可选): 内置AS的参考值配置列表。详见[Builtin AS配置](#builtin-as-configuration)。
...
- **`verify_signer_transparency`** (boolean, 可选，默认为`false`)：设置为`true`时，COCO验证器将验证Trustee AS签发的JWT token中嵌入的`signer_transparency`声明。

  > [!NOTE]
  > **背景**：当Trustee运行在不可信的服务商托管的TEE内部时，其生成的JWT签名证书缺乏内生的可信机制——验证方无法确认该证书的真实性及其与TEE的绑定关系。`signer_transparency`功能通过在Trustee AS中发布透明度声明来解决此问题，该声明将签名证书与TEE证据（如TDX Quote）绑定，并记录到Rekor v2透明度日志中。当验证方启用此选项时，可确认证书确实由TEE保护的AS生成且已公开记录，从而防止证书伪造或中间人攻击。
  >
  > 此功能仅适用于**COCO provider**的外部AS（Restful或gRPC），不适用于builtin AS模式。

  验证内容包括：
  - 证书 DER SHA-256 与透明度声明一致
  - 证据中的 report_data 与证书哈希一致（将证书绑定到TEE证据）
  - 载荷 SHA-256 与元数据摘要匹配
  - 本地验证 Rekor v2 检查点签名（ECDSA P-256）
  - 证书过期检查

  完整的透明度规范请参考 [Trustee AS signer transparency 文档](https://github.com/openanolis/trustee/blob/main/attestation-service/docs/as_signer_transparency.md)。
...
- **`as_provider`** (string)：设置为 `"ita"`
- **`as_addr`** (string，可选，默认为 `"https://api.trustauthority.intel.com"`): Intel Trust Authority API 基础 URL
- **`api_key`** (string，可选)：Intel Trust Authority API 密钥。可以直接在配置中设置，也可以通过 **`ITA_API_KEY`** 环境变量设置。如果两者都设置，配置值优先。
...
> [!NOTE]
> 如果设置了 `ITA_API_KEY` 环境变量，则可以省略配置 JSON 中的 `api_key`。TNG 会在启动时自动从环境中读取它。这是推荐的 API 密钥配置方式。

##### Builtin AS配置

当`as_type`设置为`"builtin"`时，TNG使用内置的Attestation Service在本地直接验证Evidence，而不需要连接外部的Attestation Service。这种模式适用于以下场景：
...
参考值配置指定了用于验证Evidence的参考值来源，支持以下三种模式：
...
**SLSA模式（历史兼容）**：从Rekor透明日志获取SLSA provenance

- **`type`** (string): 设置为`"slsa"`
- **`payload`** (SlsaReferenceValuePayloadConfig): 参考值payload配置

**Release Manifest模式（推荐）**：从RV release manifest bundle获取参考值

- **`type`** (string): 设置为`"release_manifest"`
- **`payload`** (SlsaReferenceValuePayloadConfig): 参考值payload配置，格式同SLSA模式
...
- **`rv_list`** (array): 参考值条目数组
  - **`id`** (string): 新格式下为release manifest中的measurement名称，如`cvm_uki`、`cvm_firmware`、`cvm_container_proxy`等，可完全自定义
  - **`version`** (string): 制品版本
  - **`type`** (string): 制品类型，如`"container"`等，可自定义
  - **`provenance_info`** (object): 来源信息
    - **`type`** (string): 来源类型，推荐`"rv-release-manifest"`（新格式）；历史兼容值为`"slsa-intoto-statements"`
    - **`rekor_url`** (string): Rekor透明日志服务器URL
    - **`rekor_api_version`** (number, 可选): Rekor API版本，支持1或2，默认为2
  - **`provenance_source`** (object): 制品来源配置。新格式必须指定此字段，支持`protocol`为`"oci"`或`"file"`（本地测试用）
    - **`protocol`** (string): 协议类型，如`"oci"`、`"file"`
    - **`uri`** (string): 来源URI，如`oci://registry/repo:tag`或`file:///path/to/bundle.json`
    - **`artifact`** (string): 制品类型，建议填写`"bundle"`
  - **`operation_type`** (string): 操作类型，`"refresh"`（刷新）或`"add"`（追加）
  - **`rv_name`** (string, 可选): 自定义参考值名称。新格式下默认使用`id`作为参考值名称
...
**示例2：使用SLSA模式提供参考值（历史兼容）**

以下示例展示了使用SLSA模式从Rekor透明日志获取参考值：
...
**示例3：使用Release Manifest模式提供参考值（推荐）**

以下示例展示了使用Release Manifest模式从OCI仓库获取release manifest bundle：
...
在Passport模式下，[Attest](#attest)配置需要包含以下字段。以下字段适用于默认 CoCo Provider（`aa_provider` = `"coco"` 或省略，`as_provider` = `"coco"` 或省略）：
...
- **`as_type`** (string，可选，默认为"restful"): Attestation Service类型。可选值："restful", "grpc"
...
- **`model`** (string)：设置为 `"passport"`
- **`aa_provider`** (string)：设置为 `"ita"`
- **`aa_addr`** (string，必填)：Attestation Agent Unix socket 地址
- **`as_provider`** (string)：设置为 `"ita"`
- **`as_addr`** (string，可选，默认为 `"https://api.trustauthority.intel.com"`): Intel Trust Authority API 基础 URL
- **`api_key`** (string，可选)：Intel Trust Authority API 密钥。也可以通过 **`ITA_API_KEY`** 环境变量设置。
- **`policy_ids`** (array [string]，可选，默认为空)：ITA 策略 ID 列表，attestation 成功必须匹配这些策略
...
在Passport模式下，[Verify](#verify)配置需要包含以下字段。以下字段适用于默认 CoCo Provider（`as_provider` = `"coco"` 或省略）：

- **`model`** (string): 设置为"passport"以启用Passport模式
- **`as_type`** (string，可选，默认为"restful"): Attestation Service类型。可选值："restful", "grpc"
- **`as_addr`** (string, 可选): Attestation Service地址，用于获取Attestation Service证书，可选；`as_addr`（或`as_type`、`as_addr`、`as_headers`字段的组合）和 `trusted_certs_paths` 至少需要指定其中之一。
- **`as_headers`** (object, 可选，默认为{})：发送到attestation service请求中的自定义头部。这在attestation service部署在需要额外认证头（如Authorization头或其他自定义头部）的认证机制后面时非常有用。
- **`policy_ids`** (array [string]): 策略ID列表
- **`trusted_certs_paths`** (array [string], 可选，默认为空)：指定用于验证Attestation Token中的签名和证书链的根CA证书路径。如果指定多个根CA证书，只要其中一个能够验证即通过。
- **`verify_signer_transparency`** (boolean, 可选，默认为`false`)：设置为`true`时，COCO验证器将验证JWT token中嵌入的`signer_transparency`声明。详见[Background Check模式](#verifybackground-check模式)中的描述。
...
- **`as_provider`** (string)：设置为 `"ita"`
- **`ita_jwks_addr`** (string，可选，默认为 `"https://portal.trustauthority.intel.com"`): Intel Trust Authority 门户 URL，用于获取 JWKS 签名密钥以进行 Token 验证
- **`policy_ids`** (array [string]，可选，默认为空)：ITA 策略 ID 列表。验证器检查 Token 的 `policy_ids_matched` 声明中是否包含所有指定的策略 ID。
...
> 有关协议设计、密钥轮换机制和异常处理的详细说明，请参考 [Peer Shared 密钥共享协议](./peer_shared_zh.md)。

TNG 支持在多个 TNG 实例之间共享 OHTTP 私钥的去中心化密钥管理模式。该模式构建于 [self_generated](#ohttp-密钥配置self_generated-模式) 模式基础之上，通过引入基于 serf 的 Gossip 分布式协议，实现了跨节点的密钥同步机制。在该模式下，每个 TNG 实例独立生成自身的 OHTTP 密钥对，并通过带有远程证明加密的 QUIC 传输层与其他集群成员共享密钥信息，确保只有经过验证的可信节点才能参与密钥交换。这种设计使得即便在无状态服务部署场景下（例如使用 HTTP 负载均衡器分发请求），集群中的任意节点都能够解密原本由其他节点公钥加密的流量，从而显著提升了系统的可用性和伸缩性。

Serf 是一种基于 Gossip 协议的成员发现和消息传播系统，它提供了去中心化的节点通信能力。在 TNG 的 `peer_shared` 模式中，每个 TNG 实例都会启动一个 Serf 客户端，所有节点间通信均通过 QUIC（基于 TLS 1.3）传输，并结合远程证明技术进行节点身份验证。

在此模式下，TNG 会：
- 每个节点独立生成自己的密钥对
- 通过带有远程证明验证的 QUIC 加密通道与其他节点交换密钥
...
- **`key`** (KeyConfig)：OHTTP 密钥管理配置。
    ...
    - **`port`** (`integer`, 可选, 默认 `8301`)：用于节点间安全通信的本地监听 UDP 端口（QUIC 传输）。
