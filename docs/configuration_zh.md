...
控制将哪些 HTTP header 从明文下游请求复制到外层 OHTTP POST 请求。这使得 Ingress 和 Egress 之间的中间设备（ALB、WAF、负载均衡器）能够读取特定的 header 用于路由、追踪或限流。

| 字段 | 类型 | 说明 |
|---|---|---|
| `request_headers` | `string[]` | 需要从下游请求复制到外层 OHTTP POST 请求的 header 名称。这些 header **不会**转发到上游服务器——它们仍然保留在 OHTTP 加密载荷中。 |
...
控制将哪些 HTTP header 从明文上游响应复制到外层 OHTTP HTTP 响应。这使得 Egress 和 Ingress 之间的中间设备能够读取特定的 header。

| 字段 | 类型 | 说明 |
|---|---|---|
| `response_headers` | `string[]` | 需要从上游响应复制到外层 OHTTP HTTP 响应的 header 名称。这些 header **不会**转发给下游客户端——它们仍然保留在 OHTTP 加密载荷中。 |
