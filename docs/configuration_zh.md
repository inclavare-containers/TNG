...
### netfilter：端口劫持方式
在该场景中，用户的server程序已监听在本机某一端口，且因业务原因不便变更端口号或为tng新增开放端口。为了让tng能够解密发往server程序端口的TCP流量，需要结合内核netfilter提供的能力，将流量重定向到tng监听的`listen_port`上。tng在解密完流量后，将TCP流量按照原先的目标发出。

此模式可以捕获来自其他节点发往本节点端口的流量（通过 PREROUTING 链），也可以捕获本地发出的出站流量（通过 OUTPUT 链）。
