...
| `as_headers` | object | `{}` | Custom headers sent to AS (e.g., Authorization) |
| `attestation_policy` | object | — | Optional for `"builtin"` type; built-in AS attestation policy configuration. Defaults to `{"type": "hardware_only"}` if omitted (the alias `{"type": "default"}` resolves to the same). Accepted `type` values: `hardware_only` (alias `default`) — only verifies hardware TEE recognition, ignores reference values (the default, suited to general-purpose deployments); `hardware_only_strict` — hardware-only reference-value posture, but TDX must be non-debug and include an event log; `hardware_with_reference_values` — trustee comprehensive appraisal against configured reference values; `hardware_strict_with_reference_values` — trustee reference-value appraisal with strict TDX non-debug and event-log requirements; `trust_all` — affirms every dimension unconditionally (debug/test only); `inline` — base64-encoded rego; `path` — path to a rego file |
