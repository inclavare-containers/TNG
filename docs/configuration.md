...
- [Remote Attestation (Common Configuration)](remote_attestation.md)
...
| `attest` | [Attest](remote_attestation.md) | None | Act as Attester at this endpoint |
| `verify` | [Verify](remote_attestation.md) | None | Act as Verifier at this endpoint |
...
> [!TIP]
> When neither `ohttp` nor `rats_tls` is specified, RA-TLS mode is used by default. Remote attestation-related fields are described in the [Remote Attestation Configuration](remote_attestation.md).
...
Remote attestation is one of the core security mechanisms in trusted computing, used to verify the runtime integrity and trustworthiness of remote systems. TNG controls remote attestation roles on each Ingress/Egress endpoint via three fields — `no_ra`, `attest`, and `verify` — supporting both Background Check and Passport models, with selectable backends such as CoCo and ITA via `aa_provider` / `as_provider`.

The full remote attestation configuration reference (models, providers, attest/verify fields, built-in AS, examples) has been extracted into a standalone document: [Remote Attestation Configuration](remote_attestation.md).
