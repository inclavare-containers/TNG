...
> [!IMPORTANT]
> wasm builtin-as **不执行真实的 TEE 证据度量**。TEE 验证器 crate（Intel TDX、AMD SEV-SNP、SGX、CSV、TPM）没有 `wasm32-unknown-unknown` 目标，因此浏览器 SDK 无法对硬件证据进行度量。wasm 上的度量能力仅限于 `trust_all`（以及 `hardware_only`，在 wasm 上退化为 trust-all）；样本参考值尽力匹配，而 `hardware_with_reference_values` / `inline` / `path` rego 策略以及 SLSA/ReleaseManifest 参考值在 wasm 上不支持（它们需要 regorus 策略引擎或 RVPS/rekor，二者均无 wasm 目标）。如需真实的 TEE 验证，请使用 `as_type: "restful"`（外部证明服务）或原生 TNG。
