...
> [!TIP]
> 可在 [参数手册](configuration_zh.md) 中查看 [Ingress](configuration_zh.md#ingress隧道入口) 和 [Egress](configuration_zh.md#egress隧道出口) 的具体配置参数。
...
原生 Verifier 将证据度量委托给外部的证明服务进程（`restful` AS）。TNG 浏览器 SDK 额外支持 **builtin-AS** 模式（`as_type: "builtin"`）：wasm SDK 内嵌了一个纯 Rust 实现的证明服务，在进程内转换并验证服务端的证明 token，无需在客户端运行外部 AS 进程。同一个 TNG 实例既签名又验证 token（闭环系统）。该路径在 wasm SDK 构建中默认启用（`__builtin-as-wasm` feature），适用于开发、演示以及不希望运行外部 AS 的场景。

> [!IMPORTANT]
> wasm builtin-as **不执行真实的 TEE 证据度量**。TEE 验证器 crate（Intel TDX、AMD SEV-SNP、SGX、CSV、TPM）没有 `wasm32-unknown-unknown` 目标，因此浏览器 SDK 无法对硬件证据进行度量。wasm 上的度量能力仅限于 `trust_all`（以及 `hardware_only`，在 wasm 上退化为 trust-all）；`Sample` 参考值仅为配置兼容性而接受，但**不被度量**（wasm builtin-as 为 trust-all，始终签发 affirming token——由于没有 TEE 验证器 / RVPS 可为 wasm 编译，wasm 上不存在参考值度量），而 `hardware_with_reference_values` / `inline` / `path` rego 策略以及 SLSA/ReleaseManifest 参考值在 wasm 上不支持（它们需要 regorus 策略引擎或 RVPS/rekor，二者均无 wasm 目标）。如需真实的 TEE 验证，请使用 `as_type: "restful"`（外部证明服务）或原生 TNG。
