...
| 方式（`-m`） | 驱动内容 |
| --- | --- |
| `tng-launch` | 以 TNG `http_proxy` 入口作为本地代理，请求经 `all_proxy` 用 `curl` 发出 |
| `tng-exec` | `tng exec`（LD_PRELOAD hook）包裹 client 命令，无需 proxy 环境变量 |
| `python-sdk` | TNG Python SDK（`tng-sdk`），包装一个 `requests` 会话 |
| `go-sdk` | TNG Go SDK（`tng-go`），作为 `openai` 客户端的 `RoundTripper` |
| `js-sdk` | TNG WASM/JS SDK，在 headless Chromium 中由 Playwright 驱动 |
...
### 1）tng-launch（http_proxy 入口；任意 HTTP 客户端经 `all_proxy`）
...
### 2）tng-exec（LD_PRELOAD hook；client 直接访问，无需 proxy 环境变量）
...
| 选项 | 说明 |
| --- | --- |
| `-m, --method` | `tng-launch` / `tng-exec` / `python-sdk` / `go-sdk` / `js-sdk` / `all`（默认 `all`） |
...
| `--proxy-port` | tng-launch 的本地代理端口（默认 41000） |
| `--tng-bin` | tng-launch/tng-exec 的 `tng` 命令前缀（默认 `cargo run --release --`） |
...
`TNG_TEST_PROXY_PORT`、`TNG_TEST_BIN`。tng-launch 方式还读取 `TNG_TEST_STARTUP_TIMEOUT`
（默认 240s）、`TNG_TEST_REQ_TIMEOUT`（默认 90s）、`TNG_TEST_AS_GRACE`（默认 15s）。
go-sdk 方式识别 `TNG_BINARY`（子进程二进制）；tng-exec 方式识别 `TNG_HOOK_LIB`（hook 库路径）；
js-sdk 方式识别 `TNG_CHROME`（浏览器二进制）。

## 输出

每种方式恰好输出一行结果：`PASS <方式>`、`FAIL <方式> <原因>` 或 `SKIP <方式> <原因>`
（如缺少前置条件）。`FAIL` 时 tng-launch/tng-exec 方式还会把日志尾部打到 stderr。任一被选方式
失败，脚本以非零码退出。
