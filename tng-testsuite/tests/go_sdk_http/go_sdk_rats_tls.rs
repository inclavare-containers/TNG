...
/// Go SDK client using rats-TLS encryption (instead of OHTTP).
...
async fn test() -> Result<()> {
    ...
    run_test!(vec![
        ...
        .boxed(),
        ShellTask {
            name: "go_rats_tls".to_string(),
            node_type: NodeType::Client,
            script: format!(
                "cd {}/rats_tls && \
                 go build -o /tmp/go_sdk_rats_tls_$$ . && \
                 TNG_BINARY={} /tmp/go_sdk_rats_tls_$$; \
                 rm -f /tmp/go_sdk_rats_tls_$$",
                test_dir,
                tng_bin.display()
            ),
