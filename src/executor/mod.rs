    ...
    for (id, add_ingress) in config.add_ingress.iter().enumerate() {
        ...
        match &add_ingress.ingress_mode {
            IngressMode::Mapping { r#in, out } => {
                ...
                let mut yamls = self::envoy::confgen::ingress::mapping::l4::gen(
                    id,
                    in_addr,
                    in_port,
                    out_addr,
    ...
    for (id, add_egress) in config.add_egress.iter().enumerate() {
        ...
        match &add_egress.egress_mode {
            EgressMode::Mapping { r#in, out } => {
                ...
                let mut yamls = self::envoy::confgen::egress::mapping::l4::gen(
                    id,
                    in_addr,
                    in_port,
                    out_addr,
            ...
            EgressMode::Netfilter {
                ...
                let mut yamls = self::envoy::confgen::egress::netfilter::l4::gen(
                    id,
                    listen_port,
                    so_mark,
                    add_egress.no_ra,
